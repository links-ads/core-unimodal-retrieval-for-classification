...
def word_freq(corpus, ngrams=[1,2,3], top=10, figsize=(10,7)):
    ...
    ## plot
    fig, ax = plt.subplots(figsize=figsize)
    # FIX: Use a list instead of a tuple for multiple columns
...
def dtf_partitioning(dtf, y, test_size=0.3, shuffle=False):
    ...
    # Get all unique values across both train and test
    all_classes = set(dtf_train[y].unique()) | set(dtf_test[y].unique())
    
    for i in all_classes:
        # Safe access with default values if a class doesn't exist in one split
        train_prop = round(dtf_train[y].value_counts(normalize=True).get(i, 0), 2)
        test_prop = round(dtf_test[y].value_counts(normalize=True).get(i, 0), 2)
        print(" ", i, " -->  train:", train_prop, "| test:", test_prop)
...
def evaluate_multi_classif(y_test, predicted, predicted_prob, figsize=(15,5)):
    ...
    # Check if y_test is already one-hot encoded (2D array)
    if len(y_test.shape) > 1 and y_test.shape[1] > 1:
        y_test_array = y_test
    else:
        # If not, convert to one-hot encoding
        y_test_array = pd.get_dummies(y_test, drop_first=False).values
    
    ## Accuracy, Precision, Recall
    accuracy = metrics.accuracy_score(y_test, predicted)
    predicted_score = np.array([np.max(row) for row in predicted_prob])
    auc = metrics.roc_auc_score(y_test, predicted_score, multi_class="ovr")
...
def fit_bow(corpus, vectorizer=None, vocabulary=None):
    ...
    plt.show()
    return {"X":X, "lst_text2tokens":lst_text2tokens, "vectorizer":vectorizer, "dic_vocabulary":dic_vocabulary, "X_names":vectorizer.get_feature_names_out()}
...
def features_selection(X, y, X_names, top=None, print_top=10):    
    ...
    for cat in np.unique(y):
        ...
        dtf_features = pd.concat([dtf_features, pd.DataFrame({"feature":X_names, "score":1-p, "y":cat})], 
                                 ignore_index=True)
...
def create_ngrams_detectors(corpus, grams_join=" ", lst_common_terms=[], min_count=5, top=10, figsize=(10,7)):
    ...
    # Remove common_terms parameter
    bigrams_detector = gensim.models.phrases.Phrases(lst_corpus, delimiter=grams_join, 
                                                     min_count=min_count, threshold=min_count*2)
    bigrams_detector = gensim.models.phrases.Phraser(bigrams_detector)
    
    trigrams_detector = gensim.models.phrases.Phrases(bigrams_detector[lst_corpus], delimiter=grams_join, 
                                                      min_count=min_count, threshold=min_count*2)
    trigrams_detector = gensim.models.phrases.Phraser(trigrams_detector)

    ## plot
    try:
        dtf_ngrams = pd.DataFrame([{"word":grams_join.join([gram if isinstance(gram, str) else gram.decode() for gram in k]), 
                                   "freq":v} for k,v in trigrams_detector.phrasegrams.items()])
    except:
        # Simpler fallback if the above fails
        dtf_ngrams = pd.DataFrame([{"word":str(k), "freq":v} for k,v in trigrams_detector.phrasegrams.items()])
    ...
    fig, ax = plt.subplots(figsize=figsize)
    # Fix: Change tuple of columns to list of columns in groupby
    sns.barplot(x="freq", y="word", hue="ngrams", dodge=False, ax=ax,
...
def plot_w2v(lst_words=None, nlp=None, plot_type="2d", top=20, annotate=True, figsize=(10,5)):
    """
    Plot word embeddings using the dimensionality reduction (2d or 3d).
    :parameter
        :param lst_words: list - ["donald trump", "china", ...]. If None, random words are picked
        :param nlp: gensim model
        :param plot_type: str - "2d" or "3d"
        :param top: num - number of similar words to plot for each lst_words
        :param annotate: bool - whether to annotate the points or not
        :param figsize: tuple - size of the figure
    """
    if nlp is None:
        print("Error: You need to provide a valid nlp model")
        return
    ...
    try:
        ...
        if lst_words is not None:
            ## add words and similar words
            lst_sim = []
            for word in lst_words:
                try:
                    lst_sim = lst_sim + [tup[0] for tup in nlp.most_similar(word, topn=top)]
                except:
                    print(f"'{word}' not found in vocabulary")
            lst_words = lst_words + lst_sim
            lst_words = list(dict.fromkeys(lst_words))  # Remove duplicates
            X = np.array([nlp[word] for word in lst_words if word in nlp.key_to_index])
        else:
            ## only random words
            n = 500 if top > 500 else top
            lst_words = list(nlp.index_to_key[:n])
            X = np.array([nlp[word] for word in lst_words])
        ...
        perplexity = min(40, len(X) - 1)  # Ensure perplexity is less than n_samples
        
        if plot_type == "2d":
            ## dimensionality reduction
            pca = manifold.TSNE(perplexity=perplexity, n_components=2, init='pca')
            X = pca.fit_transform(X)
            ## create dtf
            dtf_X = pd.DataFrame(X, columns=["x","y"])
            dtf_X["word"] = lst_words
            ## plot
            ax = fig.add_subplot(111)
            sns.scatterplot(data=dtf_X, x="x", y="y", ax=ax)
            if annotate is True:
                for i in range(len(dtf_X)):
                    ax.annotate(dtf_X["word"].iloc[i], xy=(dtf_X["x"].iloc[i], dtf_X["y"].iloc[i]), 
                               xytext=(5,2), textcoords='offset points', ha='right', va='bottom')
        else:
            ## dimensionality reduction
            pca = manifold.TSNE(perplexity=perplexity, n_components=3, init='pca')
            X = pca.fit_transform(X)
            ## create dtf
            dtf_X = pd.DataFrame(X, columns=["x","y","z"])
            dtf_X["word"] = lst_words
            ## plot
            ax = fig.add_subplot(111, projection='3d')
            ax.scatter(xs=dtf_X["x"], ys=dtf_X["y"], zs=dtf_X["z"])
            if annotate is True:
                for i in range(len(dtf_X)):
                    ax.text(dtf_X["x"].iloc[i], dtf_X["y"].iloc[i], dtf_X["z"].iloc[i], 
                            dtf_X["word"].iloc[i], size=8)
        plt.grid(True)
        plt.show()
        ...
        try:
            ...
            print("Error while suggesting alternatives")
...
:parameter
    ...
    :param annotate: bool - include word text
'''
def plot_w2v_cluster(dic_words, nlp, plot_type="2d", annotate=True, figsize=(10,5)):
    '''
    Plot words clusters with T-SNE.
    :parameter
        :param dic_words: dict - {cluster_name:[list of words]}
        :param nlp: gensim.models.keyedvectors.Word2VecKeyedVectors
        :param plot_type: str - "2d" or "3d"
        :param annotate: bool - include word text
        :param figsize: tuple - plot size
    '''
    # Check input
    if plot_type not in ["2d", "3d"]:
        raise ValueError("plot_type must be '2d' or '3d'")
    
    # Process words and create vectors
    corpus = []
    for k,v in dic_words.items():
        if len(v) > 0:
            corpus.extend(v)
    
    # Filter to words actually in the vocabulary
    words = [word for word in list(set(corpus)) if word in nlp.key_to_index]
    if len(words) == 0:
        print("No words found in the vocabulary")
        return
    
    word_vectors = [nlp[word] for word in words]
    
    # Map words to clusters
    word_clusters = {}
    for word in words:
        for cluster_name, cluster_words in dic_words.items():
            if word in cluster_words:
                word_clusters[word] = cluster_name
                break
    
    # Create matrix of vectors
    X = np.array(word_vectors)
    
    # Set up PCA or TSNE based on plot type
    try:
        if X.shape[0] < 3:
            print("Not enough data points for TSNE. Using PCA instead.")
            from sklearn.decomposition import PCA
            pca = PCA(n_components=int(plot_type[0]))
        else:
            from sklearn import manifold
            pca = manifold.TSNE(perplexity=min(40, max(5, X.shape[0]//5)), 
                                n_components=int(plot_type[0]), 
                                init='pca',
                                n_iter=2500)
        ...
        # Create dataframe
        dtf = pd.DataFrame(X, columns=["x","y"] if plot_type=="2d" else ["x","y","z"])
        dtf["word"] = words
        dtf["cluster"] = dtf["word"].map(word_clusters)
        ...
        if plot_type == "2d":
            ...
            if annotate is True:
                for i, row in dtf.iterrows():
                    ax.annotate(row["word"], (row["x"]+0.1, row["y"]), fontsize=9)
        else:
            ax = fig.add_subplot(111, projection='3d')
            for cluster in dtf["cluster"].unique():
                dtf_cluster = dtf[dtf["cluster"]==cluster]
                ax.scatter(dtf_cluster["x"], dtf_cluster["y"], dtf_cluster["z"], label=cluster)
            if annotate is True:
                for i, row in dtf.iterrows():
                    ax.text(row["x"], row["y"], row["z"], row["word"], fontsize=9)
        ...
        # Improved error recovery - using key_to_index instead of vocab
        if "'list index out of range'" in str(e):
            print("Error with word lookup. Check if all words exist in vocabulary.")
        elif "'" in str(e):
            # Try to extract the problematic word and suggest alternatives
            try:
                word = str(e).split("'")[1]
                print(f"Word '{word}' not found in vocabulary.")
                print("Similar words in vocabulary:")
                import nltk
                from nltk.metrics.distance import jaccard_distance
                from nltk.util import ngrams
                
                # Get similar words by Jaccard similarity of character trigrams
                similar_words = []
                for k in list(nlp.key_to_index.keys())[:5000]:  # Check first 5000 words only
                    if len(k) > 2 and len(word) > 2:
                        similarity = 1 - jaccard_distance(
                            set(ngrams(word, 3)), set(ngrams(k, 3))
                        )
                        if similarity > 0.7:
                            similar_words.append((k, similarity))
                
                # Show top 5 similar words
                for word, score in sorted(similar_words, key=lambda x: x[1], reverse=True)[:5]:
                    print(f"- {word} (similarity: {score:.2f})")
            except:
                print("Cannot suggest alternatives for the problematic word.")
...
'''
# Add this to a new cell to redefine the function
def utils_bert_embedding(txt, tokenizer, nlp, log=False):
    """
    Get BERT embeddings for text input.
    
    Parameters:
    - txt: string or list of strings (words)
    - tokenizer: BERT tokenizer
    - nlp: BERT model
    - log: whether to print debug info
    
    Returns:
    - numpy array with embeddings
    """
    # Handle a list of words by joining them into a string
    if isinstance(txt, list):
        txt = " ".join(txt)
    
    # Encode the text
    idx = tokenizer(txt, return_tensors="tf", padding=True, truncation=True)
    
    if log is True:
        print("Input text:", txt)
        print("Tokens:", tokenizer.convert_ids_to_tokens(idx["input_ids"][0]))
    
    # Get embeddings
    embeddings = nlp(idx)["last_hidden_state"]
    
    # Return mean of token embeddings (exclude [CLS] and [SEP] by taking average)
    return embeddings[0].numpy().mean(axis=0)
...
:return
    vector or matrix 
'''
# Add this to a new cell after the previous one
def embedding_bert(x, tokenizer, nlp, log=False):
    """
    Create BERT embeddings for different types of text inputs.
    
    Parameters:
    - x: string, list of strings, or list of list of strings
    - tokenizer: BERT tokenizer
    - nlp: BERT model
    - log: whether to print debug info
    ...
    - numpy array with embeddings
    """
    # Single string --> vector (size,)
    if isinstance(x, str):
        X = utils_bert_embedding(x, tokenizer, nlp, log)
        
    # List of strings --> matrix (n, size)
    elif isinstance(x, list) and all(isinstance(s, str) for s in x):
        X = np.array([utils_bert_embedding(s, tokenizer, nlp, log) for s in x])
        
    # List of lists of words --> matrix (n mean vectors, size)
    elif isinstance(x, list) and isinstance(x[0], list):
        X = np.array([utils_bert_embedding(" ".join(lst), tokenizer, nlp, log) for lst in x])
        
    else:
        raise ValueError("Input format not supported. Must be string, list of strings, or list of list of strings.")
...
def explainer_similarity_classif(tokenizer, nlp, dic_clusters, txt_instance, token_level=False, top=5, figsize=(20,10)):
    ...
    # Create embeddings for each cluster's words
    y_embeddings = []
    for v in dic_clusters.values():
        if len(v) > 0:  # Check if word list is not empty
            embs = embedding_bert(v, tokenizer, nlp)
            y_embeddings.append(embs)
    
    # Concatenate all embeddings
    y = np.vstack([emb for emb in y_embeddings if emb.size > 0])
    
    # Get embeddings for text instance
    ...
    perplexity = min(40, max(5, len(M)-1))
    pca = manifold.TSNE(perplexity=perplexity, n_components=2, init='pca')
    ...
    for k,v in dic_clusters.items():
        if len(v) > 0:  # Skip empty lists
            size = current_idx + len(v)
            dtf_group = pd.DataFrame(y[current_idx:size], columns=["x","y"], index=v)
            dtf_group["cluster"] = k
    ...
    fig, ax = plt.subplots(figsize=figsize)
    sns.scatterplot(data=dtf, x="x", y="y", hue="cluster", ax=ax)
    if ax.get_legend() is not None and len(ax.get_legend().texts) > 0:
        ax.get_legend().texts[0].set_text(None)
    ...
    ## add txt_instance
    if token_level is True:
        # Get tokens without using encode directly (which caused the error)
        encoding = tokenizer(txt_instance, return_tensors='pt')
        tokens = tokenizer.convert_ids_to_tokens(encoding['input_ids'][0])[1:-1]  # Skip special tokens
        dtf = pd.DataFrame(X, columns=["x","y"], index=tokens)
        # Filter out subword tokens and short tokens
        dtf = dtf[~dtf.index.str.contains("#", na=False)]
